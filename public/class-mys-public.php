...
class CRM_HUB_MYS_Public {
    ...
    public function mys_services_get_referencia_api()
    {
        ...
        if($product->is_type('simple') && get_field('tipo_producto')[0] == 'normal' && $product->get_manage_stock()){
            ...
            // Consulta bodega de Bogota
            $body_api_001 = json_encode(array(
            ...
            $responde_001 = wp_remote_post($url, $args_001);

            $stock_q_001 = json_decode($responde_001['body']);

            // Consulta bodega de Medellin
            $body_api_008 = json_encode(array(
                "referencia" => $product->get_sku(),
                "bodega" => "008"
            ));
            ...
            $responde_008 = wp_remote_post($url, $args_008);

            $stock_q_008 = json_decode($responde_008['body']);

            $stock_q = $stock_q_008 + $stock_q_001;
        ...
        if($product->is_type('variable') && get_field('tipo_producto')[0] == 'normal'){
            foreach ($product->get_available_variations('objects') as $product_woo){
                ...
                // Consulta de la bodega de Bogota
                $args_api_001 = array(
                    "referencia" => $product_woo->get_sku(),
                    "bodega" => "001"
                );

                $body_api_001 = json_encode($args_api_001);
                ...
                $stock_q_001 = intval(json_decode($responde_001['body']));
                ...
                $body_api_008 = json_encode($args_api_008);
                ...
                $stock_q_008 = intval(json_decode($responde_008['body']));

                $stock_q = $stock_q_008 + $stock_q_001;

                echo '<pre>';
                var_dump($stock_q);
                var_dump($stock_q_008);
                var_dump($stock_q_001);
                echo '</pre>';
        ...
        // Funcion para cambiar precio de las referencias VARIABLES.
        if($product->is_type('variable') && get_field('tipo_producto')[0] == 'normal'){
